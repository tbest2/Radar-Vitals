...
def update():
    ...
    # print(f"Bytes waiting in serial port: {bytes_waiting}")

    # print(f"Dataport.in_waiting = {Dataport.in_waiting}")
    # if Dataport.in_waiting > 0:
    #     raw = Dataport.read(Dataport.in_waiting)
    #     print("Raw data (first 32 bytes):", raw[:32])
    #     # Put the raw data back to a global buffer for parsing in readAndParseData68xx()
    #     # Or call your parser directly with raw
    # else:
